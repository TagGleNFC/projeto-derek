...
function initializePresetPage() {
    ...
    setupTutorialVideo(preset); // Configura o novo player de vídeo tutorial
    updateBackgroundVideos(preset.backgroundVideo); // Carrega os vídeos de background
}

// Função para atualizar os vídeos de background
...
function populatePageData(preset) {
    ...
    // Preenche a nova seção "O Que Você Vai Receber"
    const toolsList = document.querySelector('.tools-list ul');
    if (toolsList && preset.packageItems) {
        toolsList.innerHTML = preset.packageItems.items.map(item => `
            <li>
    ...
    // Configura o botão de compra final
    ...
    if (finalPurchaseButton) {
        finalPurchaseButton.addEventListener('click', () => {
            const encodedTitle = encodeURIComponent(preset.title);
            window.open(`https://kirvano.com/checkout?preset=${encodedTitle}`, "_blank");
        });
    }

    // Configura o botão de rolagem
...
// --- LÓGICA DO VÍDEO TUTORIAL (SIMPLIFICADO) ---
function setupTutorialVideo(preset) {
    const tutorialVideoPlayer = document.getElementById('tutorial-video-player');

    // Se o elemento de vídeo e a fonte do vídeo existirem no preset...
    if (tutorialVideoPlayer && preset.tutorialVideo) {
        // ...apenas define o caminho do vídeo.
        // Os atributos (controls) no HTML cuidam do resto.
        tutorialVideoPlayer.src = preset.tutorialVideo;
    } else if (tutorialVideoPlayer) {
        // Se não houver vídeo para o preset, esconde o contêiner.
        tutorialVideoPlayer.parentElement.style.display = 'none';
    }
}


// --- LÓGICA DO PLAYER COM WAVEFORM (Permanece a mesma) ---
